...
Three of the seven [What to build](#what-to-build) items have now been built or run, and that list says what each measured and what is still untested. [`scripts/mpfb_probe.py`](https://github.com/Xander-Rudolph/game-asset-engine/blob/main/scripts/mpfb_probe.py) ran item 1, MPFB2 in the container's headless `bpy` ([MakeHuman and MPFB2](#makehuman-and-mpfb2)). [`scripts/daz_inventory.py`](https://github.com/Xander-Rudolph/game-asset-engine/blob/main/scripts/daz_inventory.py) is item 2. No Daz content library is installed here, so it has run only on its self-test, a synthetic library and a few third-party copies of Genesis morph files, never on a figure file. `scripts/render_sheet.py` now opens `.blend` files and sets shape keys and rig properties per pose, item 4. The lip sync side is in the [talking portraits](/guide/talking-portraits) guide. No Daz content was installed, imported or added to the repository, and no importer was run. Statements measured on 2026-09-16 while building carry no claim id; each says which script produced it, or sits in a passage that does.

The research itself was read, not run: Daz's licence, product and file-list pages, the DSON specification, the Diffeomorphic importer's source and wiki, forum threads, a third-party copy of a few `.dsf` files, the licence files of the alternatives and the free Quaternius and KayKit downloads. No importer was tried in the container. This repository's `scripts/render_sheet.py` and `scripts/cleanup.py` were read on 2026-09-16, before `render_sheet.py` gained `.blend` input. The verified claims behind this page are in [research/claims/daz-genesis.json](https://github.com/Xander-Rudolph/game-asset-engine/blob/main/research/claims/daz-genesis.json) on GitHub.
:::
...
| Read `.duf` and `.dsf` | Any Python: JSON behind gzip <!-- DAZ-035, DAZ-070 --> | Read from specification and source. `scripts/daz_inventory.py` reads `.dsf` with the standard library; not yet run on a figure file |
...
| EEVEE render and sheet | `scripts/render_sheet.py` in the container | Exists: bones, shape keys and rig properties per pose, and `.blend` input since 2026-09-16; not tried on a Genesis import |
...
Ranked. None of it duplicated an existing script when proposed. The notes under items 1, 2 and 4 say what was built or run on 2026-09-16, what it measured and what is still untested; items 3, 5, 6 and 7 have not been started.
...
2. **`scripts/daz_inventory.py`.** Standard-library `gzip` and `json` only. <!-- DAZ-035, DAZ-070 --> For a content directory outside the repo, print per figure the vertex and face counts, bone names and count, morph names grouped by prefix (`eCTRLv`, `facs_ctrl_v`, `facs_bs_`, `facs_cbs_`, `facs_jnt_`, `pJCM`, `body_cbs_`), and whether each `_div2` `.dsf` holds deltas or only an `hd_url`. <!-- DAZ-074, DAZ-081, DAZ-082, DAZ-084, DAZ-086 --> It needs no Blender, desktop or AI stage, turns this page's community counts into measured ones, and settles two open questions below.
   ...
   **Built on 2026-09-16; not yet run on Daz content.** [`scripts/daz_inventory.py`](https://github.com/Xander-Rudolph/game-asset-engine/blob/main/scripts/daz_inventory.py) does the above with only `gzip` and `json`, matches folder names without regard to case, because Daz's own URIs mix `Daz 3D` and `DAZ 3D`, and refuses any path in or above this repository. Its `--selftest` passed 55 checks in 0.50 s, and it read a synthetic library from `--sample` and a few third-party copies of Genesis 8, 8.1 and 9 morph and controller files. No Genesis content library is installed on this machine, so it has read no figure file, and this page's vertex, polygon and bone counts are still community figures. Running it needs Genesis 9 Starter Essentials, and Genesis 8 Starter Essentials for 8.1, installed outside the repository. Also untested: a real gzip-compressed `.dsf`, the `.dhdm` check on a real tree, and speed on a whole library.
...
- **Whether Diffeomorphic registers under the pip `bpy` module and runs with `--background`.** Not tried. MPFB2, another extension, registered under the container's pip `bpy` on 2026-09-16 once added from a local extension repository ([MakeHuman and MPFB2](#makehuman-and-mpfb2)); that does not show Diffeomorphic will.
...
- **Whether Genesis 8.1's `_div2` `.dsf` files carry useful base-resolution deltas**, or mostly point at `.dhdm` data. <!-- DAZ-074, DAZ-084 --> One data point since: on 2026-09-16 `scripts/daz_inventory.py` found 272 base deltas as well as an `hd_url` in a third-party copy of Genesis 8 Female 8.1's `facs_bs_BrowDownLeft_div2`. One file does not settle it, and whether those deltas are useful was not judged.
...
- **Settled: MPFB2 face packs load without a UI.** Experiment 1 ran on 2026-09-16: in the container's `bpy` 4.5.9 all three packs installed from their zips and the 15 Meta/Oculus visemes loaded as shape keys ([MakeHuman and MPFB2](#makehuman-and-mpfb2)). Still open: loading the Microsoft-style and ARKit packs, and how the visemes look with a skin material.
