...
- **Fitting a MakeHuman hair OBJ to Genesis 9.** This was handed over by the session that ran the hair work, measured by it earlier on 2026-09-23, and not re-measured or written up anywhere else. A MakeHuman hair OBJ is not in the body's frame and needs its `.mhclo` weights and offsets applied, which moved bob02 up 5.2 cm. MakeHuman's helper geometry skews a sphere fit, so fit on the `g body` faces only. The fitted bob02's fringe still cut into the Genesis 9 forehead, which `--declip-skip` hid but did not fix. The proper fix, a shrinkwrapped donor head with weights, was never built. The ten CC0 system-asset hair OBJs are extracted in `input/_devtools/makehuman/hair/` (gitignored). This is the starting point for the owner's new direction on 2026-09-23, "go with hair objs instead. same with beards". Cost: code, then minutes of CPU per style.
