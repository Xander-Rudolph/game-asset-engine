...
This lists what was built on the `research/sfm-lipsync-daz` branch (2026-09-15 to 2026-09-16) but not tested for real, and the decisions left to the owner. Remove an item when it is tested or decided, and say what settled it in the commit.
...
| Claude reading a Daz render | Allow, or keep refusing | The EULA's AI clause names chat models. `daz-genesis.md:379`, daz-figure steps 4 and 5, `daz_import_probe.py:252` |
...
| Restated ComfyUI arithmetic | Reword "Mirrors" with a link, or derive it at run time | `compose_mouths.kontext_crop` restates GPL-3.0 code almost token for token |
...
- **A `.dbz` import.** Without one, two `_HD3` morphs logged Missing geonode, so visemes may differ. The owner exports `Genesis 9.dbz` from Daz Studio beside the `.duf`; compare `build --fit DBZFILE` with `g9_visemes_build.json`. Cost: needs the owner and Daz Studio.
...
- **`daz_import_probe.py` beyond one run.** Render numbers come from one option set, `DEFAULT_LIBRARY` ignores MODELS_DIR, and the host-side kill never fired. `build` with `--material-method BSDF` and `FBX_COMPATIBLE`, a Toon figure, Basic Wear (76 case-only references) and a wrong `--content-dir`; `pkill -STOP -f scripts/render_sheet.py` during `render --timeout 120`. Cost: minutes of CPU each.
- **A person judging renders.** The only by-eye reading is Claude's, on clay. The owner views `g9_cage` and `--materials` Kat sheets; rerun the full cage render and relabel stale `g9_kat*` files. Cost: needs the owner, about 25 min of CPU.
