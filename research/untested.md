...
| Claude reading a Reallusion render | Settled 2026-09-23: allowed. The owner said Claude should be allowed to read Reallusion renders, as for Daz on 2026-09-22. Nothing else changed: none of it is committed and none of it reaches an AI stage | The Content EULA bars "AI-generated output" and the base-model licence bars "machine learning"; neither names a chat model, and the reading is that looking at a render is not training or generation. `skills/character-source/SKILL.md` step 4 |
| Guards for Reallusion, Poser and Character Generator content | Settled 2026-09-23: not built. The owner said to build no AI guards unless something requires them, and none of these licences requires tooling: they bar the use itself | The rule lives in `skills/character-source/SKILL.md`. `scripts/cleanup.py keep` still guards only Daz |
| Whether the repo wants a Virt-A-Mate import path at all | Build it in `scripts/`, keep it experimental, or drop it | A creator's CC BY covers the strand shapes, but a `CustomScalp` `.vab` embeds a `DAZMesh` block of Daz Genesis 2 geometry, so a VaM-derived asset cannot ship as 3D data on that licence alone. `vam-assets.md`, Licences |
| The procedural hair generator as the route for hair and beards | Settled 2026-09-23: called off. The owner looked at the settled generator on the figure beside a Virt-A-Mate groom at the same framing and chose sourced hair meshes instead | `scripts/make_hair.py` and `scripts/bake_hair.py` still run and still ship, and `make_hair.py` kept the settle landed that day; the `hair-mesh` skill was removed; `hair-cards.md` records what was measured and why it was called off |
...
- **The `character-source` render route.** `scripts/render_sheet.py` has never been given a Character Creator, Poser or Character Generator FBX. Once the owner has downloaded the free CC base characters, render one front and side and look at the bones, shape keys and framing. Cost: a download, then minutes of GPU.
- **Questions for Reallusion,** in writing, because its FAQ and its binding EULA disagree. Does the Content EULA's bar on "embedded content within applications or online services" cover a character in a game build, when its visible FAQ answers game distribution with "Yes"? Do the 2026 terms reach a Character Creator 3 or iClone 7 perpetual licence bought before them? May a game ship a character sculpted from the free CC base, when the free base FAQ says "Your derivative characters are fully yours" but Section 5 grants commercial use only to characters made "without using any content from Reallusion"? What licence does the CC5 Free Resource Pack carry? Its only embedded-content answer, for iClone 8 and Character Creator 4, is withdrawn from the page. `character-tools.md`, CHT-004, CHT-006, CHT-066, CHT-069, CHT-071. Cost: needs the owner to write to them.
- **A question for Bondware:** which of the two Poser 14 EULA texts governs, the installer's or the manual's? They differ on Vendor Resources. CHT-051. Cost: needs the owner to write.
- **A question for Autodesk:** what licence covers Character Generator output? No current terms page names the service, and only forum posts grant game use. CHT-031, CHT-033. Cost: needs the owner to write.
- **Character Generator, signed in.** Generate and download one character, and check it has a rigged skeleton and not "just locators", as users reported from July 2025 (CHT-070). Do it before Mudbox's one-year and monthly subscriptions end sale on 7 November 2026, after which entry costs about US$2,010 a year (CAS-027). Cost: an Autodesk subscription and minutes.
- **The CC5 Free Resource Pack's licence, in the app.** Which items are CC Components can only be seen inside Character Creator on Windows (CHT-066). Cost: needs the owner and Windows.
- **ExpressionPlus against the repo's mouth set.** Once a free CC base is imported, list its shape keys and map them to the Rhubarb letters in `docs/reference/lip-sync.md`. The "AccuLips & ExPlus" profile holds 63 blend shapes and the AccuLips visemes (DAZ-107), but what names the free FBX carries is unknown. Cost: minutes of CPU after the download.
- **The free cast, rendered.** Quaternius's base characters and Dragon_Evolved (CC0) and OpenGameArt's CC BY reptile have never been through `scripts/render_sheet.py`, and their styles do not match (`character-assets.md`, What to build). Cost: downloads, then minutes of GPU.
- **Daz prices, by hand.** Every Daz store price in `character-assets.md` is unverifiable, because Daz's terms forbid automated access. The owner reads the 41 products on the shopping list in a browser (CAS-010). Cost: needs the owner.
- **Whether Reallusion's iClone dragons animate in Character Creator 5 without iClone 8,** which decides whether the Reallusion cast costs US$277 or up to US$599 more (CAS-015, CAS-019). Cost: needs Windows and the pack.
- **Fitting a MakeHuman hair OBJ to Genesis 9.** This was handed over by the session that ran the hair work, measured by it earlier on 2026-09-23, and not re-measured or written up anywhere else. A MakeHuman hair OBJ is not in the body's frame and needs its `.mhclo` weights and offsets applied, which moved bob02 up 5.2 cm. MakeHuman's helper geometry skews a sphere fit, so fit on the `g body` faces only. The fitted bob02's fringe still cut into the Genesis 9 forehead, which `--declip-skip` hid but did not fix. The proper fix, a shrinkwrapped donor head with weights, was never built. The ten CC0 system-asset hair OBJs are extracted in `input/_devtools/makehuman/hair/` (gitignored). This is the starting point for the owner's new direction on 2026-09-23, "go with hair objs instead. same with beards". The shortlist of hair and beard meshes to fit is in `docs/reference/hair-meshes.md`. Cost: code, then minutes of CPU per style.
- **The first `terms-recheck` run.** The maintainer skill has loaded but never done a full pass. The one lesson so far: read what a page shows, not its raw HTML, because Reallusion's old FAQ sits in markup hidden from readers, and two checks read it as live. Cost: an hour of reading.

## Virt-A-Mate assets

- **The import path is experimental and nothing is in `scripts/`.** `output/hair/_exp/vam_read.py` reads a `.var` and `output/hair/_exp/groom2.py` fits its guides to Genesis 9, and both produced the renders in `vam-assets.md`, but neither has a `--help`, a report format, a `--check` or a test. Nothing was baked to an OBJ, nothing was given an atlas, and no second figure was tried. Cost: a day, and the owner decision above first.
- **The transform was fitted on one creator.** Scale 0.9899 and offset (0, -0.30, -6.92) cm come from one `scooped` package and were checked against four more by the same creator, so it is one authoring figure. Read a package by a different creator and see whether it lands. Cost: a download the owner makes in a browser, then minutes.
- **One package would not read.** Every scalp slot in the first hair `.vab` of `verytoxic.Boss_Fades.2.var` holds zero points, and why was not established. Cost: an hour with the reader.
- **The CC BY reading has had no two-lens check.** It rests on the `licenseType` field in each `meta.json`, not on the licence text, so `vam-assets.md` states only the conservative conclusion. Run the `licence-audit` check before anything is relied on beyond "do not ship". Cost: minutes.
